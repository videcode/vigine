...
namespace subsystem{
	namespace core{
		...
		template<typename tSlot, typename... tArgs> requires api::cEvent<tSlot, tArgs...>
		...
		template<typename tSlot, typename... tArgs> requires api::cEvent<tSlot, tArgs...>
		class EventHelper: public api::iEventHelper<tSlot, tArgs...>{
			public:
				...
				void slot(tSlot* slotIn) override{
					this->pEvent_->slot(slotIn);
				}

				void slotRemove(tSlot* slotIn) override{
					this->pEvent_->slotRemove(slotIn);
				}

				void on(tArgs... argv) override{
					this->pEvent_->on(argv...);
				}

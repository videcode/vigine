...
void Surface::presentModeKHR(vk::PhysicalDevice &device) {
  std::vector<vk::PresentModeKHR> presentModes =
      device.getSurfacePresentModesKHR(this->surface);
  if (presentModes.size() > 0)
    this->presentMode = vk::PresentModeKHR::eFifo;
  else
    std::runtime_error(
        "ERROR: physical device does not suppot any present mode");
}

void Surface::capabilitiesKHR(vk::PhysicalDevice &device) {

  vk::Result res = device.getSurfaceCapabilitiesKHR(this->surface,
  ...
  if (res == vk::Result::eSuccess) {
    ...
    std::runtime_error(
        "ERROR: device.getSurfaceCapabilitiesKHR return bad result");
}

void Surface::imageUsagesFlags() {
  vk::ImageUsageFlags supportedImageUsages =
      this->imageUsage & this->surfaceCapabilities.supportedUsageFlags;
...
void Surface::setFormat(vk::PhysicalDevice &device) {
  ...
  std::vector<vk::SurfaceFormatKHR> surfaceFormatArr =
      device.getSurfaceFormatsKHR(this->surface);
  if (surfaceFormatArr.size() > 0) {
    if ((1 == surfaceFormatArr.size()) &&
        (vk::Format::eUndefined == surfaceFormatArr[0].format)) {
      ...
      for (vk::SurfaceFormatKHR &item : surfaceFormatArr) {
        if (item.format == desiredFormat.format &&
            item.colorSpace == desiredFormat.colorSpace) {

...
void Swapchain::init(std::shared_ptr<vk::Device> logicDevice,
                     Surface &surface) {
  // swapchain
...
void Swapchain::imageIndex(std::shared_ptr<vk::Device> logicDevice) {
  ...
  vk::Result res = logicDevice->acquireNextImageKHR(
      this->swapchain, 2000000000, nullptr, nullptr, &imageIndex);
...
void Swapchain::createImages(std::shared_ptr<vk::Device> logicDevice,
                             Surface &surface) {
  ...
  for (int i = 0; i < this->vImages.size(); i++) {
    ...
    vk::Result res = logicDevice->createImageView(&createInfo, nullptr,
                                                  &this->vImageViews[i]);

...
public:
  ...
  void perspective(float fovy, float aspect, float near, float far) {
    ...
    this->proj_ = glm::perspective(glm::radians(this->fovy_), this->aspect_,
  ...
  void lookAt(glm::vec3 posIn, glm::vec3 targetIn, glm::vec3 upIn) {
    ...
    this->view_ =
        glm::lookAt(posIn,    // Camera is at camPos, in World Space
  ...
  void lookAt() {
    this->view_ =
        glm::lookAt(this->pos_,    // Camera is at camPos, in World Space
  ...
  void keyPress(int keyCode) {
    ...
    case 40: { // d
      ...
      this->view_ = this->view_ * glm::mat4_cast(q);
      this->directMove_ =
          glm::inverse(glm::mat4_cast(q)) * glm::vec4(this->directMove_, 1.0);
    } break;
    }
...
template <> class Impl<api::iCamera> {
public:
  static std::unique_ptr<api::iCamera> make() {
    return std::unique_ptr<api::iCamera>(
        static_cast<api::iCamera *>(new subsystem::Camera()));
  }
